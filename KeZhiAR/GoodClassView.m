 ...
 */
-(NSArray *)goodClassArray{
    if (!_goodClassArray) {
        _goodClassArray = [NSMutableArray array];
    }
    
    return _goodClassArray;
}
...
- (void)loadNewShops
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if ([self.db open]) {
            ...
            // 1.执行查询语句
            FMResultSet *resultSet = [self.db executeQuery:@"SELECT * FROM KeZhi_GoodClass"];
            // 2.遍历结果
            while ([resultSet next]) {
                GoodClassModel * video = [[GoodClassModel alloc]init];
...
- (UICollectionView *)collectionView
{
    if (_collectionView == nil) {
        ...
        flowLayout.itemSize = CGSizeMake(fDeviceWidth-20, fDeviceWidth/3+10);
        ...
        //注册cell
        [_collectionView registerClass:[GoodClassCollectionViewCell class] forCellWithReuseIdentifier:@"cell"];
...
-(NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    ...
    NSLog(@"goodClass:%lu",(unsigned long)[self.goodClassArray count]);
    _collectionView.mj_footer.hidden = self.goodClassArray.count == 0;
    
    return [_goodClassArray count];
}
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    GoodClassCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identify forIndexPath:indexPath];
    [cell sizeToFit];
    
    //获取标题
    _video = self.goodClassArray[indexPath.item];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //点击跳转页面
    GoodClass_VideoViewController *view = [[GoodClass_VideoViewController alloc]init];
    view.selectedItemNum = indexPath.item;
    view.shortVideo = _goodClassArray[indexPath.item];
    view.db = self.db;
