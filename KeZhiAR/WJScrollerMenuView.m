...
-(void)loadDataAndView
{
    ...
    if (_showArrayButton) {
        ...
        arrowButton.frame = CGRectMake(_navigationTabBarWidth+10, 15, ARROW_BUTTON_WIDTH/2, self.bounds.size.height/2);
        arrowButton.backgroundColor = self.backgroundColor;
        //修改图片
        [arrowButton setBackgroundImage:[[UIImage imageNamed:@"sy_01"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
...
- (void)setCurrentIndex:(NSInteger)currentIndex
{
    if (_items.count>0) {
        ...
        [UIView animateWithDuration:.2f animations:^{
            //设置下划线的长短
            _line.frame = CGRectMake(button.frame.origin.x + 25.0f, _line.frame.origin.y, [_itemsWidth[currentIndex] floatValue] - 50.0f, _line.frame.size.height);
        }];
    }
}
...
- (void)arrowBtnClick:(UIButton *)button
{
    ...
    if (!_PopupView) {
        ...
        _PopupView.selectedColor=[UIColor colorWithRed:70.0/255 green:218.0/255 blue:193.0/255 alpha:1.0];
